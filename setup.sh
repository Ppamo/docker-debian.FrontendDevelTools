...
usage(){
	echo "./setup.sh [start|clean|clean_image]"
}
...
run(){
	...
	if [ "$CONTAINER" ]; then
		...
		if [ "$CONTAINER" ]; then
			...
			docker run --privileged=true -di -p 3000:3000 --name "$MACHINE_NAME" -v $PWD/repos:/opt/repos -v $PWD/scripts:/tmp/scripts "$IMAGENAME:$IMAGETAG" $CMD
			CONTAINER=$(docker ps | grep "$MACHINE_NAME" | awk '{ print $1 }')
